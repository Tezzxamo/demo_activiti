...
public class ProcessManager {
    ...
    public void suspendProcessDefinitionByName(String processDefinitionName) {
        ...
        ActivitiUtil.instance().getRepositoryService().suspendProcessDefinitionById(processDefinition.getId());
    }
    ...
    public void cascadeSuspendProcessDefinitionByName(String processDefinitionName, boolean cascade, Date suspensionDate) {
        ProcessDefinition processDefinition = VerificationUtils.checkProcessDefinitionByName(processDefinitionName);
        ActivitiUtil.instance().getRepositoryService().suspendProcessDefinitionById(processDefinition.getId(), cascade, suspensionDate);
    }
    ...
    public void activateProcessDefinitionByName(String processDefinitionName) {
        ProcessDefinition processDefinition = VerificationUtils.checkProcessDefinitionByName(processDefinitionName);
        ActivitiUtil.instance().getRepositoryService().activateProcessDefinitionById(processDefinition.getId());
    }
    ...
    public void cascadeActivateProcessDefinitionByName(String processDefinitionName, boolean cascade, Date activationDate) {
        ProcessDefinition processDefinition = VerificationUtils.checkProcessDefinitionByName(processDefinitionName);
        ActivitiUtil.instance().getRepositoryService().activateProcessDefinitionById(processDefinition.getId(), cascade, activationDate);
    }
    ...
    public Boolean getProcessDefinitionStatusByName(String processDefinitionName) {
        ...
        ProcessDefinition processDefinition = ActivitiUtil.instance().getRepositoryService().createProcessDefinitionQuery()
    ...
    public Map<String, Boolean> getProcessConfigStatusMap() {
        ...
        // 放入所有的激活状态的流程定义
        ActivitiUtil.instance().getRepositoryService().createProcessDefinitionQuery()
                .active()
                .latestVersion()
                .list()
    ...
    public Collection<ProcessDefinitionVO> getProcessDefinitions() {
        return Collections.unmodifiableList(ActivitiUtil.instance().getRepositoryService().createProcessDefinitionQuery()
                        .latestVersion()
                        .list())
                .stream()
    ...
    public void deleteProcessDefinitionByName(String processDefinitionName) {
        ProcessDefinition processDefinition = VerificationUtils.checkProcessDefinitionByName(processDefinitionName);
        ActivitiUtil.instance().getRepositoryService().deleteDeployment(processDefinition.getDeploymentId());
    }
    ...
    public void cascadeDeleteProcessDefinitionByName(String processDefinitionName) {
        ProcessDefinition processDefinition = VerificationUtils.checkProcessDefinitionByName(processDefinitionName);
        ActivitiUtil.instance().getRepositoryService().deleteDeployment(processDefinition.getDeploymentId(), true);
    }
    ...
    public ProcessDefinitionVO deployProcessDefinition(String processDefinitionName, String bpmnPath) {
        ...
        try {
            // 创建部署对象并部署
            Deployment deploy = ActivitiUtil.instance().getRepositoryService().createDeployment()
